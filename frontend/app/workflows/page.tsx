...
import {
  Activity, Play, Pause, Square, RefreshCw, Plus, Terminal,
  LayoutList, Grid3x3, Columns3,
} from 'lucide-react'
import ExpandableCard from '@/components/ExpandableCard'
import type { DealerEntry } from '@/lib/types'
import {
  groupWorkflows, parseWorkflowKey, workflowKey,
  getBoardLane, type WorkflowBoardLane,
...
export default function WorkflowsPage() {
  const { dealers, mutate } = useDealers()
  const { workflows }       = useWorkflows()
  const { pauseDealer, resumeDealer, stopDealer, dealNextFor, startDealer } = useEngineActions()
  ...
  const groupedWorkflows = useMemo(() => groupWorkflows(workflows, dealers), [workflows, dealers])

  // Persist layout preference
  useEffect(() => {
    const saved = window.localStorage.getItem(LAYOUT_STORAGE_KEY) as LayoutMode | null
    if (saved === 'list' || saved === 'grid' || saved === 'board') setLayoutMode(saved)
  }, [])
  useEffect(() => { window.localStorage.setItem(LAYOUT_STORAGE_KEY, layoutMode) }, [layoutMode])

  // Default workflow selection
  useEffect(() => {
    if (selectedWorkflowKey || workflows.length === 0) return
  ...
  const workflowExpanded = (key: string) =>
    groupedWorkflows.length === 1 ? true : !!expandedWorkflows[key]
  ...
  const handleLaunch = async () => {
    ...
    setLaunchError('')
    try {
      await startDealer(workspace.trim(), name, version || 'v1')
      setWorkspace('')
      mutate()
    } catch {
      setLaunchError('Failed to launch dealer. Check the workspace path and workflow name.')
    } finally {
      setLaunching(false)
    }
  }

  const statusBadge = (dealer: DealerEntry) => {
    const label = dealer.is_paused ? 'paused' : dealer.status
    const cls =
  ...
  const laneTitle = (lane: WorkflowBoardLane) =>
    lane === 'running' ? 'Running' : lane === 'paused' ? 'Paused' : lane === 'error' ? 'Error' : 'Other'
  ...
  function DealerCard({ dealer }: { dealer: DealerEntry }) {
    return (
      <div className="rounded-xl border border-slate-700/50 bg-slate-900/20">
        <div className="px-4 py-3 border-b border-slate-700/40">
          <div className="text-sm font-semibold text-slate-300 truncate">{dealer.dealer_id}</div>
        </div>
        <div className="p-4 space-y-3">
          {([
            ['Workflow',    `${dealer.workflow}/${dealer.version}`],
            ['Workspace',   dealer.workspace],
            ['Current Card', dealer.current_card_id ?? 'N/A'],
            ['Completed',   `${dealer.completed_total} cards (${dealer.cycles_completed} cycles)`],
            ['Last Updated', dealer.last_updated ? new Date(dealer.last_updated).toLocaleTimeString() : 'N/A'],
          ] as [string, string][]).map(([label, value]) => (
            <div key={label} className="flex items-center justify-between">
              <span className="text-sm text-slate-400">{label}</span>
              <span className="font-mono text-xs text-slate-300 max-w-48 truncate text-right">{value}</span>
            </div>
          ))}
          ...
          <div className="flex gap-2 pt-3 border-t border-slate-700/50">
            <button
              onClick={() => dealNextFor(dealer.dealer_id).then(() => mutate())}
            ...
            >
              <RefreshCw className="w-3.5 h-3.5" /> Deal Next
            </button>
            {dealer.is_paused ? (
              <button
                onClick={() => resumeDealer(dealer.dealer_id).then(() => mutate())}
                className="flex-1 flex items-center justify-center gap-1 px-3 py-1.5
              ...
              <button
                onClick={() => pauseDealer(dealer.dealer_id).then(() => mutate())}
                className="flex-1 flex items-center justify-center gap-1 px-3 py-1.5
            ...
            <button
              onClick={() => stopDealer(dealer.dealer_id).then(() => mutate())}
              className="flex-1 flex items-center justify-center gap-1 px-3 py-1.5
    ...
    <div className="w-full px-6 py-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="ml-auto text-xs text-slate-500 font-mono">
          {dealers.length} dealer{dealers.length !== 1 ? 's' : ''} · {workflows.length} workflow{workflows.length !== 1 ? 's' : ''}
        </span>
      </div>
      ...
      <div className="glass-card rounded-2xl p-5 space-y-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="text-sm font-semibold text-slate-300">Launch New Dealer</h2>
        </div>
        <div className="flex flex-wrap gap-3">
          <input
            type="text"
            placeholder="Workspace path  (e.g. ./workspace_3)"
            value={workspace}
      ...
      {groupedWorkflows.length === 0 ? (
        <div className="glass-card rounded-2xl p-8 text-center">
          <p className="text-slate-400">No dealers running.</p>
          ...
          {groupedWorkflows.map(group => (
            <div key={group.key} className="glass-card rounded-2xl p-4">
              ...
              {group.dealers.length === 0 ? (
                <p className="text-xs text-slate-500 italic">No active dealers for this workflow.</p>
              ) : (
                <div className="space-y-3">
                  {group.dealers.map(d => <DealerCard key={d.dealer_id} dealer={d} />)}
                </div>
              )}
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-4">
          {(['running', 'paused', 'error', 'other'] as WorkflowBoardLane[]).map(lane => {
            const laneGroups = groupedWorkflows.filter(g => getBoardLane(g) === lane)
            return (
              <div key={lane} className={`rounded-2xl border p-4 ${laneTone(lane)}`}>
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <span className="text-xs">{laneGroups.reduce((s, g) => s + g.dealers.length, 0)}</span>
                </div>
                <div className="space-y-3">
                  {laneGroups.length === 0 ? (
                    ...
                    <div key={group.key} className="rounded-lg border border-slate-700/40 bg-slate-950/30 p-3">
                      ...
                      <div className="mt-2 text-[11px] text-slate-500">{group.dealers.length} active dealer(s)</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="space-y-6">
          {groupedWorkflows.map(group => (
            <ExpandableCard
              ...
              expandedContent={
                group.dealers.length === 0 ? (
                  <div className="text-sm text-slate-500 italic">No active dealers for this workflow.</div>
                ) : (
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    {group.dealers.map(d => <DealerCard key={d.dealer_id} dealer={d} />)}
                  </div>
            ...
            >
              <div className="px-4 py-3 text-xs text-slate-500">
                {group.dealers.length} active dealer{group.dealers.length !== 1 ? 's' : ''}
              </div>
            </ExpandableCard>
          ))}
        </div>
