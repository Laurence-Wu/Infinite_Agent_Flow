...
from core.tmux_detector import PromptDetector
...
class TmuxManager(TmuxBase):
    ...
    def start(self) -> None:
        ...
        then in a background thread: accept consent (Ctrl+Y), wait for the
        prompt, and paste AGENT_LOOP.md.
    ...
    def _wait_for_any_output(self, timeout: float = 30.0) -> bool:
        """Block until the pane has any non-empty content (consent screen ready)."""
    ...
    def _startup_sequence(self) -> None:
        """
        Background thread: accept consent → wait for prompt → paste loop file.
        Sets self._starting = False when done (or on failure).
        """
        try:
            # Wait until gemini has printed the consent screen, then accept it
            self._wait_for_any_output(timeout=30)
            self._run("send-keys", "-t", self._session, "C-y", "")

            # Wait for the agent to be ready (polls pane — no fixed sleep)
